//------------------------------------------------------------------------------
// File: Mightex_ClassicSource.cpp
//
...
#include "VideoShow.h"
#include <initguid.h>
#include "IMightex_ClassicSource.h"
...
CMightex_VideoShow::CMightex_VideoShow()
{
	...
	//g_pRC = NULL;
	//g_pMF = NULL;
    pMightex_ClassicSourceFilter = NULL;
	mIMightex_ClassicSource = NULL;
	pSC = NULL;
	...
	pMightex_ClassicSourceFilterOut = NULL;
	old_ghApp = NULL;
	isPreviewing = FALSE;
	isCapturing = FALSE;
	isCompressing = FALSE;
...
CMightex_VideoShow::~CMightex_VideoShow()
{
	...
	if(pMightex_ClassicSourceFilter) pMightex_ClassicSourceFilter->Release();
	if(mIMightex_ClassicSource) mIMightex_ClassicSource->Release();//Can't the pointer be released?
	if(pSC) pSC->Release();
	...
	//if(pMightex_ClassicSourceFilterOut) pMightex_ClassicSourceFilterOut->Release();
...
{	
    ...
    // Add Mightex_ClassicSource filter to our graph.
    hr = g_pGraph->AddFilter(pMightex_ClassicSourceFilter, L"Mightex_ClassicSource Filter");
    if (FAILED(hr))
    {
        pMightex_ClassicSourceFilter->Release();
        return hr;
    }
	...
	//hr = ConnectFilters(g_pGraph, pMightex_ClassicSourceFilter, pSmartTee);
    if (FAILED(hr))
    {
        return hr;
    }
	...
	pMightex_ClassicSourceFilterOut = GetOutPin(pMightex_ClassicSourceFilter, 0);
...
{
	mIMightex_ClassicSource->GetCameraControl(cameraCtrl, cameraGlobalCtl);
	if(cameraCtrl.deviceID == 0)
		return E_FAIL;
...
*/
    ...
    hr = CoCreateInstance (CLSID_Mightex_ClassicSource, NULL, CLSCTX_INPROC_SERVER, 
		IID_IBaseFilter, reinterpret_cast<void**>(&pMightex_ClassicSourceFilter));
    if (FAILED(hr))
        return hr;
	...
	hr = pMightex_ClassicSourceFilter->QueryInterface(IID_IMightex_ClassicSource, (void **)&mIMightex_ClassicSource);
    if (FAILED(hr))
        return hr;

	hr = pMightex_ClassicSourceFilter->QueryInterface(IID_IAMStreamConfig, (void **)&pSC);
    if (FAILED(hr))
        return hr;
...
HRESULT CMightex_VideoShow::AddCompressorsToList(CStringList &m_CompressorList)
{
    ...
    while(1)
    {
		...
		hr = pMightex_ClassicSourceFilterOut->Connect(pCompressorIn, pmt);
        if(!FAILED(hr))
...
HRESULT CMightex_VideoShow::StopPreview()
{
	...
	else
	{
		...
		hr = g_pGraph->Disconnect(pMightex_ClassicSourceFilterOut);
		if (FAILED(hr))
		{
			return hr;
		}
...
{	
	...
	hr = g_pGraph->Connect(pMightex_ClassicSourceFilterOut, pSmartTeeIn);
	if (FAILED(hr))
	{
		return hr;
	}
...
HRESULT CMightex_VideoShow::Snapshot(char * TargetFile, BOOL SavetoFileNeeded, BOOL SaveAsJPEG, BOOL AppendDataTime, BOOL SwitchSkipModeNeeded, int SaveFileCount, int SavedCount)
{
	...
	mIMightex_ClassicSource->Snapshot(TargetFile, SaveAsJPEG, AppendDataTime, SaveFileCount);

	return S_OK;
}

HRESULT CMightex_VideoShow::GetCameraControl(CCameraControl &cameraCtrl)
{
	CCameraGlobalControl cameraGlobalCtl;
	mIMightex_ClassicSource->GetCameraControl(this->cameraCtrl, cameraGlobalCtl);
...
{
	mIMightex_ClassicSource->GetPhysicalCameraFrameRate(cameraCtrl.actualFrameRate);
...
{	
	...
	mIMightex_ClassicSource->SetStreamFrameRate(frameRate);
...
{
	HRESULT hr = mIMightex_ClassicSource->SetCameraWorkMode(WorkMode);
	if (hr == NOERROR)
...
HRESULT CMightex_VideoShow::SetResolution(int width, int height, int bin)
{
	...
	HRESULT hr = mIMightex_ClassicSource->SetResolution(width, height, bin);
	if (hr == NOERROR)
	{
...
HRESULT CMightex_VideoShow::SetExposureTime(int exposureTime)
{
	...
	mIMightex_ClassicSource->SetExposureTime(exposureTime);
	if (!isPreviewing)
		mIMightex_ClassicSource->SetResolution(cameraCtrl.width, cameraCtrl.height, cameraCtrl.bin);
...
HRESULT CMightex_VideoShow::SetGains(int redGain, int greenGain, int blueGain)
{
	...
	mIMightex_ClassicSource->SetGains(redGain, greenGain, blueGain);
...
HRESULT CMightex_VideoShow::SetXYStart(int xStart, int yStart)
{
	...
	mIMightex_ClassicSource->SetXYStart(xStart, yStart);
...
HRESULT CMightex_VideoShow::OpenPropertyPages(HWND ghApp)
{
	...
	hr = pMightex_ClassicSourceFilter->QueryInterface(IID_ISpecifyPropertyPages,(void **)&pSpec);
	if(hr == S_OK)
	{
		...
		if(hr == S_OK)
			hr = OleCreatePropertyFrame(ghApp, 30, 30, NULL, 1,
				(IUnknown **)&pMightex_ClassicSourceFilter, cauuid.cElems,
...
HRESULT CMightex_VideoShow::SetGPIOConifg(unsigned char ConfigByte)
{
	return mIMightex_ClassicSource->SetGPIOConifg(ConfigByte);
}

HRESULT CMightex_VideoShow::SetGPIOInOut(unsigned char OutputByte, unsigned char *InputBytePtr)
{
	return mIMightex_ClassicSource->SetGPIOInOut(OutputByte, InputBytePtr);
}
